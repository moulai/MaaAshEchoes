...
- **日常-午后茶憩-茶憩角色 1**：与指定的第一个角色进行茶憩互动，**若选择自行截图设置角色，请创建或修改`resource/image/edit/午后茶憩_茶憩角色1.png`以指定角色**，详见下方的“使用说明”。**请确保该角色能正常“再来一杯”且所有茶憩对话已经完成**。设置“无免费次数时允许使用通用邀请函”配合“重复次数”使用，可解放双手反复喝茶提升角色默契值。
...
- **日常-升级烙痕**：尝试选择低等级烙痕进行升级，若无可升级烙痕会自动跳过，用于完成每日任务。
- **日常-活动日常任务**：随版本更新的活动日常任务及每日奖励领取。
- **漫巡-以置顶刻印漫巡**：以置顶刻印配置进行全自动漫巡，使用前请先自行调整好全自动选项（如最大深度）。支持自动刷新支援烙痕、使用上次编组基点、点亮推荐技能。
- **周常-打一次拟合回归**：执行一次拟合回归任务，支持选择队伍。**请确保已启用自动战斗，并且队伍中1-4号位无支援角色**。
